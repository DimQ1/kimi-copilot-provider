...
const CHILD_SCHEMA_SLOTS: readonly ChildSchemaSlot[] = [
  { key: '$defs', kind: 'map' },
...
function removeIrrelevantStructureKeys(node: Record<string, unknown>, newType: JsonSchemaType): void {
  if (newType !== 'object') {
    for (const key of OBJECT_STRUCTURE_KEYS) delete node[key];
  }
  if (newType !== 'array') {
    for (const key of ARRAY_STRUCTURE_KEYS) delete node[key];
  }
}
...
function inferTypeFromValues(values: unknown[]): JsonSchemaType {
  ...
  if (types.length === 1) {
    ...
    if (onlyType === undefined) throw new Error('Cannot infer JSON Schema type from an empty enum.');
    return onlyType;
  }
...
function normalizeInferredTypes(types: Set<JsonSchemaType>): JsonSchemaType[] {
  ...
  const order: JsonSchemaType[] = ['string', 'number', 'integer', 'boolean', 'object', 'array', 'null'];
  return order.filter((t) => normalized.has(t));
}
